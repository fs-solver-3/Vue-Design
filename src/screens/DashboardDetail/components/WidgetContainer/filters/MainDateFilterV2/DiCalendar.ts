...
export default class DiCalendar extends Vue {
  ...
  onMainDateFilterModeChanged(newMode: MainDateFilterMode) {
  ...
  private apply(): CalendarData {
    ...
    Log.debug('currentData::', this.currentCalendarData);
    this.submittedPresetDateRange = this.currentCalendarData.chosenDateRange!;
    this.oldCalendarData = this.currentCalendarData;
  ...
  private showDatePicker() {
    Log.debug('currentcalendar::', this.currentCalendarData);

    this.currentCalendarData = cloneDeep(this.oldCalendarData);
    Log.debug('oldcalendar::', this.oldCalendarData);
    this.errorMessage = '';
  ...
  //todo: wrap object events return
  private handleSelectStartTime(startRange: any) {
    try {
      ...
      Log.debug('DiCalendar::handleSelectStartTime::exception::', e.message);
    }
  }
  ...
  private handleSelectEndTime(endRange: any) {
    try {
      ...
      Log.debug('DiCalendar::handleSelectEndTime::exception::', e.message);
    }
  }
}
