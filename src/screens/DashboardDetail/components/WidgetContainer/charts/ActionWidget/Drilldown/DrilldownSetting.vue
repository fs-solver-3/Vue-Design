<template>
  <div :id="containerId">
    <div class="popover-panel">
      <div class="header">
        <div class="d-inline mr-1 btn-ghost-alter" @click="handleResetDrilldown">
          <b-icon-house-door />
        </div>
        <template v-for="(path, index) in drilldownPaths">
          <div :key="index" class="d-inline btn-ghost-alter" @click.stop="rollbackQueryTo(index)">
            <div class="d-inline">{{ path }}</div>
            <div class="d-inline">{{ divider }}</div>
          </div>
        </template>
        <div class="d-inline btn-ghost-alter">
                ...
                <template slot-scope="{ data }">
                  <template v-if="isGroupDate(data)">
                    <DataListing :records="data.children" key-for-display="displayName" key-for-value="field">
                      <div slot-scope="{ row }" class="custom-display-option d-flex flex-row justify-content-center align-items-center btn-ghost-alter">
                        ...
                        <div :id="genIdSeeMore(row)" class="col-2 custom-more-icon cursor-pointer" tabindex="-1" @click.stop="handleClickSeeMore(row)">
                          <img alt="more" src="@/assets/icon/ic_arrow_right.svg" />
                        </div>
                      </div>
                    </DataListing>
                  </template>
...
import { ChartInfo, Field, FieldRelatedFunction, Group, WidgetId } from '@core/domain/Model';
import { DateFunctionTypes, FunctionFamilyTypes, GroupedField, LabelNode, SortTypes, Status } from '@/shared';
...
export default class DrilldownSetting extends Vue {
  ...
  private drilldown(fieldDetailInfo: FieldDetailInfo, toFieldRelatedFn: FieldRelatedFunction): void {
    if (Drilldownable.isDrilldownable(this.currentQuery)) {
      ...
      const drilldownData = new DrilldownData(this.valueSelected, toFieldRelatedFn, fieldDetailInfo.displayName ?? '');
      ...
      DrilldownDataStoreModule.saveDrilldownData({
        id: this.widgetId,
        newPath: `${this.header} (${this.valueSelected})`,
        query: newQuery
      });
  ...
  private rollbackQueryTo(index: number): void {
