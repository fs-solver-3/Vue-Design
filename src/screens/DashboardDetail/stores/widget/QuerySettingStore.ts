...
import { PivotTableQuerySetting, WidgetId, Zoomable } from '@core/domain/Model';
...
export class QuerySettingStore extends VuexModule {
  ...
  applySort(payload: { id: WidgetId; sortAsMap: Map<string, SortDirection> }) {
    ...
    if (query && Sortable.isSortable(query)) {
...
export const QuerySettingModule = getModule(QuerySettingStore);
