...
import { DataModule } from './DataStore';
...
export class RenderControllerStore extends VuexModule {
  ...
  async requestRender(id: number): Promise<void> {
    if (this.enable) {
      ...
      DataModule.setStatusRendered(id);
    }
  }
  ...
  private nextRender() {
    ...
    if (id) {
      this.currentIdRendering = id;
      DataModule.setStatusRendering(id);
...
export const RenderControllerModule = getModule(RenderControllerStore);
