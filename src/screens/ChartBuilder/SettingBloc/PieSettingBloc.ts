...
import { PieVizSetting, SeriesVizData, SeriesVizSetting, VizSetting, VizSettingData } from '@core/domain';
import { cloneDeep } from 'lodash';
import { SettingKey } from '@/screens/ChartBuilder/SettingBloc/Event';

export class PieSettingBloc extends SettingBloc<SeriesVizData> {
  ...
  getVizSetting(): VizSetting<SeriesVizData> {
    return new PieVizSetting(this.currentVizSettingData);
  }

  clone(): SettingBloc {
    return new PieSettingBloc(cloneDeep(this.currentValueAsMap), cloneDeep(this.currentVizSettingData));
  }
