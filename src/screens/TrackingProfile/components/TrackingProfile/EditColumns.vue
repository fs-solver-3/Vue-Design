...
import { ScrollConfigs, Stores } from '@/shared';
...
export default class EditColumns extends Vue {
  ...
  constructor() {
    ...
    this.scrollOption = ScrollConfigs;
    ...
    profileSettingInfo.columns.forEach(col => {
      if (col instanceof NestedColumn) {
        ...
        col.nestedColumns.map(nestedCol => {
          const field = new Field(profileSettingInfo.dbName, profileSettingInfo.name, `\`${col.name}.${nestedCol.name}\``, nestedCol.className);
          nestedCols.push(new UserProfileCheckboxGroupOption(nestedCol.name, nestedCol.displayName, true, field));
        });
        result.push(...nestedCols);
      } else {
        const field = new Field(profileSettingInfo.dbName, profileSettingInfo.name, col.name, col.className);
