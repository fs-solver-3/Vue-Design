...
import { HorizontalGridSetting, PivotTableQuerySetting, PivotTableVizData, PivotTableVizSetting, TableVizData, VerticalGridSetting } from '@core/domain/Model';
...
export default class PivotTable extends BaseWidget {
  ...
  private get vizSetting(): PivotTableVizSetting | null {
  ...
  private async handleLoadSubRow(payload: ToggleCollapseData): Promise<void> {
    try {
      ...
      const children: RowData[] = await this.getSubRows(rowData);
      ...
      Log.debug('handleLoadSubRow::error', ex);
      this.hideLoading();
    }
  }

  private async getSubRows(rowData: RowData): Promise<RowData[]> {
  ...
  private getPivotSettingStyle(setting?: PivotTableVizSetting | null) {
    const currentSetting: PivotTableVizData = setting?.options ?? {};
    ...
    const cssObject = {
      ...
      '--footer-white-space': currentSetting.total?.label?.style?.isWordWrap ? 'pre-wrap' : void 0,
