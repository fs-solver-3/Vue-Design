...
import { ConditionData, ConfigType, FunctionData, WidgetType } from '@/shared';
...
export class QuerySettingResolver {
  ...
  toQuerySetting(chartType: WidgetType, configsAsMap: Map<ConfigType, FunctionData[]>, filterAsMap: Map<Id, ConditionData[]>): QuerySetting {
  ...
  canBuildQuerySetting(chartType: WidgetType, configsAsMap: Map<ConfigType, FunctionData[]>, filterAsMap: Map<Id, ConditionData[]>): boolean {
    const creator = this.mapCreator.get(chartType);
    if (creator) {
