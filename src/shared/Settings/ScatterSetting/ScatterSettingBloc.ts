...
import { ScatterQuerySetting, ScatterVizSetting, SeriesVizData, VizSetting } from '@core/domain';
import { cloneDeep } from 'lodash';
import { SettingKey } from '@/screens/ChartBuilder/SettingBloc/Event';

export class ScatterSettingBloc extends SettingBloc<SeriesVizData> {
  ...
  getVizSetting(): VizSetting<SeriesVizData> {
    return new ScatterVizSetting(this.currentVizSettingData);
  }

  clone(): SettingBloc {
    return new ScatterSettingBloc(cloneDeep(this.currentValueAsMap), cloneDeep(this.currentVizSettingData));
  }

  protected onVizResponseChanged(event: ChangeVizResponse): SeriesVizData {
    this.currentValueAsMap.set('xAxis[0].title.text', (event.querySetting as ScatterQuerySetting).xAxis.name);
    this.currentValueAsMap.set('yAxis[0].title.text', (event.querySetting as ScatterQuerySetting).yAxis.name);
