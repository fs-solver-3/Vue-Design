...
import { HeatMapQuerySetting, HeatMapVizSetting, SeriesVizData, VizSetting } from '@core/domain';
import { cloneDeep } from 'lodash';
import { SettingKey } from '@/screens/ChartBuilder/SettingBloc/Event';

export class HeatMapSettingBloc extends SettingBloc<SeriesVizData> {
  ...
  getVizSetting(): VizSetting<SeriesVizData> {
    return new HeatMapVizSetting(this.currentVizSettingData);
  }

  clone(): SettingBloc {
    return new HeatMapSettingBloc(cloneDeep(this.currentValueAsMap), cloneDeep(this.currentVizSettingData));
  }

  protected onVizResponseChanged(event: ChangeVizResponse): SeriesVizData {
    this.currentValueAsMap.set('xAxis[0].title.text', (event.querySetting as HeatMapQuerySetting).xAxis.name);
    this.currentValueAsMap.set('yAxis[0].title.text', (event.querySetting as HeatMapQuerySetting).yAxis.name);
