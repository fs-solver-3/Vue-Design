...
import { BellCurveVizSetting, SeriesVizData, VizSetting } from '@core/domain';
import { cloneDeep } from 'lodash';
import { SettingKey } from '@/screens/ChartBuilder/SettingBloc/Event';

export class BellCurveSettingBloc extends SettingBloc<SeriesVizData> {
  ...
  getVizSetting(): VizSetting<SeriesVizData> {
    return new BellCurveVizSetting(this.currentVizSettingData);
  }

  clone(): SettingBloc {
    return new BellCurveSettingBloc(cloneDeep(this.currentValueAsMap), cloneDeep(this.currentVizSettingData));
  }
