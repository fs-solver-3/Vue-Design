...
import { SeriesVizData, TreeMapVizSetting, VizSetting } from '@core/domain';
import { cloneDeep } from 'lodash';
import { SettingKey } from '@/screens/ChartBuilder/SettingBloc/Event';

export class TreeMapSettingBloc extends SettingBloc<SeriesVizData> {
  ...
  getVizSetting(): VizSetting<SeriesVizData> {
    return new TreeMapVizSetting(this.currentVizSettingData);
  }

  clone(): SettingBloc {
    return new TreeMapSettingBloc(cloneDeep(this.currentValueAsMap), cloneDeep(this.currentVizSettingData));
  }
