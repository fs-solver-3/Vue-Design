...
export class FormattingOptions {
  ...
  static buildTableOptions(tables: SlTreeNodeModel<TableSchema>[], formatType: ConditionalFormattingType): DropdownData[] {
    return tables.map(table => {
      return {
        ...
        options:
          table.children
            ?.filter(column => FormattingOptions.isShowColumn(column, formatType))
            .map(column => {
