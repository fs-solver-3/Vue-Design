...
export abstract class UserAdminRepository {
  ...
  abstract searchV2(from: number, size: number, isActive?: boolean): Promise<UserSearchResponse>;
...
export class UserAdminRepositoryImpl extends UserAdminRepository {
  ...
  searchV2(from: number, size: number, isActive?: boolean): Promise<UserSearchResponse> {
    const params = {
      from: from,
      size: size,
      // eslint-disable-next-line @typescript-eslint/camelcase
  ...
  getSuggestedUsers(request: SearchUserRequest): Promise<UsersResponse> {
    return this.httpClient.post(`user/profile/suggest`, request);
  }
}
